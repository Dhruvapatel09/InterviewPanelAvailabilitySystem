...
export class RecruiterPanelByAllComponent implements OnInit{
  ...
  onClickUpdate(slotId: number) {
    if (window.confirm('Are you sure you want to book this slot?')) {
      this.recruiterService.updateInterviewSlot(slotId).subscribe({
        next: (response: ApiResponse<InterviewSlots[]>) => {
          if (response.success) {
            ...
            // Check if we need to adjust pagination after update
            if (this.recruiterDetailsPagination.length === 1 && this.pageNumber > 1) {
              // Move to the previous page if we're on the last page with one record
              this.pageNumber--;
            }
          } else {
            console.error('Failed to update slot', response.message);
          }
        },
        error: (error) => {
